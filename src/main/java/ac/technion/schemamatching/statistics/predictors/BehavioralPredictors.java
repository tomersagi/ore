...
public class BehavioralPredictors implements Statistic {
	...
	public boolean init(String instanceDescription, MatchInformation mi) {
		...
		double outlierLimit = 2*ds.getStandardDeviation()+ds.getMean();
		overallMaxDiff = 0.0;
		for (double diff :maxDiff.values())
			overallMaxDiff = Math.max(overallMaxDiff,(diff>outlierLimit ? 0 : diff));
	...
	/**
	 * 
	 * @return max of diff after removing outliers
	 */
