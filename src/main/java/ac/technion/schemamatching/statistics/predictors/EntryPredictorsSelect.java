 ...
 *
 */
public class EntryPredictorsSelect implements Statistic {

    private static String name = "Entry Predictors";
    private String[] header;
    private ArrayList<String[]> data = new ArrayList<String[]>();
    private String predName = "";
    public EntryPredictorsSelect(String predName)
    {
		...
		*/
        ...
        header = new String[]{"InstanceDesc", predName};
    }
    ...
    public boolean init(String instanceDesc, MatchInformation mi) {
        ...
        for (int row =0; row < confM.length; row++)
            for (int col =0; col < confM[row].length; col++)
            {
                ...
                if (this.predName.equals("DBN")){
                    res[1] = new Double(dbn).toString();
//					assert(dbn<=1.0);
                }
                else if (this.predName.equals("DSM")){
                    res[1] = new Double(dsm).toString();
//					assert(dsm<=1.0);
                }
                else if (this.predName.equals("DLA")){
                    res[1] =  new Double(dla).toString();
//					assert(dla<=1.0);
                }
                else if (this.predName.equals("RNV")){
                    res[1] = new Double(rnv).toString();
                }
                else if (this.predName.equals("RRV")){
                    res[1] = new Double(rrv).toString();
                }
                else if (this.predName.equals("CNV")){
                    res[1] = new Double(cnv).toString();
                }
                else if (this.predName.equals("CRV")){
                    res[1] = new Double(crv).toString();
                }
                else if (this.predName.equals("CRV")){
                    res[1] = new Double(val).toString();
                }
                data.add(res);
                assert(val<=1.0);
            }
